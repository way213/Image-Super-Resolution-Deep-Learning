...
The mapping layer in the original FSRCNN uses a Conv(3x3) filter. We propose 2 modified architectures: 1) A Conv(3x1) filter followed by a Conv(1x3) filter, 2) a Conv(1x1) filter followed by a Conv(3x3) filter, then a Conv(1x1) filter (1xN → Nx1).
