...
def main() -> None:
    ...
    model = FSRCNN(config.upscale_factor, config.exp_number).to(config.device)
    ...
    results_dir = os.path.join(f"results/test/exp_number_{config.exp_number}_x{config.upscale_factor}_{config.validate_data}")
    ...
    for index in range(total_files):
        ...
        # Only reconstruct the Y channel image data.a
        with torch.no_grad():
            start = time.time()
            sr_y_tensor = model(lr_y_tensor).clamp_(0, 1.0)
            inference_time = time.time() - start
            inference.append(inference_time)
        ...
        if not cv2.imwrite(sr_image_path, sr_image * 255.0):
            print("Could not save")

    print(f"PSNR: {total_psnr / total_files:4.2f}dB.\n")
    inference_average = np.round(sum(inference) / len(inference), 4)
    print(f"Average Inference Time: {inference_average}")
